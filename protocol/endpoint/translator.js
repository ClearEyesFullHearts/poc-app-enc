...
class Translator {
  ...
  get alsEncKeyValidator() {
    return this.#EncPKValidator;
  }

  get alsSigKeyValidator() {
    return this.#SigPKValidator;
  }
  ...
  async request(isAnonymous, token, cipheredBody, proof, {
    ...
    if (!user) {
      throw new EndpointError('No user present');
    }
    if ((isAnonymous && user !== 'anonymous') || (!isAnonymous && user === 'anonymous')) {
      throw new EndpointError('Anonimity mismatch');
    }

    if (ttl && Date.now() > ttl) {
      throw new EndpointError('Token is expired');
    }
